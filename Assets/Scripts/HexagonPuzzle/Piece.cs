...
namespace HexagonPuzzle
{
    public class Piece : MonoBehaviour
    {
        ...
        #region Property
        {
            ...
            set
            {
                for (int i = 0; i < SharedPropertiesHelper.SharedProperties.GridColors.Count; i++)
                {
                    if (SharedPropertiesHelper.SharedProperties.GridColors[i].Equals(value))
                    {
                        colorIndex = i;
                        SpriteRenderer.color = SharedPropertiesHelper.SharedProperties.GridColors[i];
                        break;
                    }
                }
            }
...
#endif
            if (randomizeColor)
                Color = SharedPropertiesHelper.SharedProperties.GridColors[Random.Range(0, SharedPropertiesHelper.SharedProperties.GridColors.Count)];
